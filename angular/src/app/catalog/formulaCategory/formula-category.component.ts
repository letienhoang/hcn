...
export class FormulaCategoryComponent implements OnInit, OnDestroy {
  ...
  showAddModal() {
    const ref = this.dialogService.open(FormulaCategoryDetailComponent, {
      header: 'Thêm mới danh mục công thức',
      width: '70%',
    });
    ...
    ref.onClose.subscribe((data: FormulaCategoryDto) => {
      if (data) {
        this.loadData();
        this.notificationService.showSuccess('Thêm danh mục công thức thành công');
        this.selectedItems = [];
      }
    });
  }
  ...
  showEditModal() {
    ...
    const ref = this.dialogService.open(FormulaCategoryDetailComponent, {
      ...
      header: 'Cập nhật danh mục công thức',
      width: '70%',
    });
    ...
    ref.onClose.subscribe((data: FormulaCategoryDto) => {
      if (data) {
        this.loadData();
        this.notificationService.showSuccess('Cập nhật danh mục công thức thành công');
        this.selectedItems = [];
      }
    });
  }
